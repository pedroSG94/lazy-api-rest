...
class GenerateInvariableJavaFiles:
    def copy_invariable_java_files(self, module_name, package_name, code_folder, base_url):
        ...
        copyfile(source_java_files_folder + os.sep + "LibraryCallback.java",
                 code_folder + os.sep + str(module_name).title().replace(" ", "") + "Callback.java")
        copyfile(source_java_files_folder + os.sep + "Retrofit2ServiceImp.java",
                 code_folder + os.sep + "Retrofit2ServiceImp.java")
    ...
    def __modify_package(self, module_name, package_name, code_folder, base_url):
        ...
        Utils.replace_content_in_file(
            code_folder + os.sep + str(module_name).title().replace(" ", "") + "Callback.java", "com.example.library",
            package_name)
        Utils.replace_content_in_file(
            code_folder + os.sep + str(module_name).title().replace(" ", "") + "Callback.java", "Library",
            str(module_name).title().replace(" ", ""))

        Utils.replace_content_in_file(code_folder + os.sep + "Retrofit2ServiceImp.java", "com.example.library",
                                      package_name)

        Utils.replace_content_in_file(code_folder + os.sep + "Constants.java", "com.example.library", package_name)
        Utils.replace_content_in_file(code_folder + os.sep + "Constants.java", "http://example_url.com", base_url)
